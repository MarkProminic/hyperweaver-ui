        ...
        <div className="card h-100">
          <div className="card-body">
            <h3 className="h5 mb-4 d-flex align-items-center gap-2">
              ...
              <span>{t('host.networkStorageSummary.networkInterfaces')}</span>
            </h3>
            {Array.isArray(networkInterfaces) && networkInterfaces.length > 0 ? (
              <>
                <div className="table-responsive">
                  <table className="table">
                    <thead>
                      <tr>
                        <th>{t('host.networkStorageSummary.interface')}</th>
                        <th>{t('host.networkStorageSummary.class')}</th>
                        <th>{t('host.networkStorageSummary.state')}</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                {networkInterfaces.length > 5 && (
                  <p className="small text-muted mt-2">
                    {t('host.networkStorageSummary.showing', { total: networkInterfaces.length })}
              ...
              <p className="text-muted">{t('host.networkStorageSummary.noInterfaces')}</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="card h-100">
            <div className="card-body">
              <h3 className="h5 mb-4 d-flex align-items-center gap-2">
                ...
                <span>{t('host.networkStorageSummary.storageSummary')}</span>
              </h3>
              {Object.keys(storageSummary).length > 0 ? (
                <div className="table-responsive">
                  <table className="table">
                    <tbody>
                      <tr>
                        <td>
                          <strong>{t('host.networkStorageSummary.zfsPools')}</strong>
                        </td>
                        <td>{storageSummary.pools?.length || 0}</td>
                      </tr>
                      <tr>
                        <td>
                          <strong>{t('host.networkStorageSummary.datasets')}</strong>
                        </td>
                ...
                <p className="text-muted mb-4">
                  {t('host.networkStorageSummary.storageUnavailable')}
                </p>
              )}
            </div>
          </div>
