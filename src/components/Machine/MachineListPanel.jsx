...
import RowActions from './MachineListRowActions';
...
const MachineListPanel = ({ currentServer, user, onSelect }) => {
  ...
  const { hosts: topoHosts } = useTopologyFeed({ scope: 'host' });
  const topoGraph = topoHosts[0]?.graph || null;
              ...
              <div className="border rounded p-2" key={row.name}>
                <div className="d-flex justify-content-between align-items-start flex-wrap gap-2">
                  <div>
                    ...
                    {topoGraph && (
                      <div className="mt-1">
                        <TopologyMini graph={topoGraph} machineName={row.name} />
                      </div>
                    )}
                  </div>
                  <RowActions
